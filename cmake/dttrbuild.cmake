include(CMakeParseArguments)

macro(dttr_find_sdl3)
	find_package(PkgConfig REQUIRED)
	if(NOT TARGET PkgConfig::SDL3)
		set(PKG_CONFIG_USE_STATIC_LIBS OFF)
		pkg_check_modules(SDL3 REQUIRED IMPORTED_TARGET sdl3)
	endif()
endmacro()

macro(dttr_find_sdl3_mixer)
	find_package(PkgConfig REQUIRED)
	if(NOT TARGET PkgConfig::SDL3_MIXER)
		set(PKG_CONFIG_USE_STATIC_LIBS OFF)
		pkg_check_modules(SDL3_MIXER REQUIRED IMPORTED_TARGET sdl3-mixer)
	endif()
endmacro()

function(dttr_configure_runtime_dependencies)
	set(DTTR_TOOLCHAIN_TARGET "i686-w64-mingw32")
	set(DTTR_SDL3_RUNTIME_DIR
		"${CMAKE_SOURCE_DIR}/.toolchain/sdl3/${DTTR_TOOLCHAIN_TARGET}/bin"
	)
	set(DTTR_SDL3_MIXER_RUNTIME_DIR
		"${CMAKE_SOURCE_DIR}/.toolchain/sdl3_mixer/${DTTR_TOOLCHAIN_TARGET}/bin"
	)
	set(DTTR_SDL3_RUNTIME_DLL "${DTTR_SDL3_RUNTIME_DIR}/SDL3.dll")

	set(DTTR_SDL_RUNTIME_DLLS
		"${DTTR_SDL3_RUNTIME_DLL}"
		"${DTTR_SDL3_MIXER_RUNTIME_DIR}/SDL3_mixer.dll"
		PARENT_SCOPE
	)
	set(DTTR_SDL3_RUNTIME_DLL "${DTTR_SDL3_RUNTIME_DLL}" PARENT_SCOPE)
endfunction()

function(dttr_configure_ffmpeg)
	set(DTTR_FFMPEG_DIR "${CMAKE_SOURCE_DIR}/.toolchain/ffmpeg")
	set(DTTR_FFMPEG_LIBS avformat avcodec avutil swscale swresample)
	set(DTTR_FFMPEG_TARGETS ${DTTR_FFMPEG_LIBS})
	list(TRANSFORM DTTR_FFMPEG_TARGETS PREPEND "ffmpeg_")

	file(GLOB DTTR_FFMPEG_RUNTIME_DLLS CONFIGURE_DEPENDS
		"${DTTR_FFMPEG_DIR}/bin/*.dll"
	)
	list(SORT DTTR_FFMPEG_RUNTIME_DLLS)
	if(NOT DTTR_FFMPEG_RUNTIME_DLLS)
		message(FATAL_ERROR
			"FFmpeg runtime DLLs missing in ${DTTR_FFMPEG_DIR}/bin. "
			"Run toolchain/bootstrap first."
		)
	endif()

	foreach(DTTR_FFMPEG_LIB DTTR_FFMPEG_TARGET IN ZIP_LISTS
		DTTR_FFMPEG_LIBS
		DTTR_FFMPEG_TARGETS
	)
		if(NOT EXISTS "${DTTR_FFMPEG_DIR}/lib/lib${DTTR_FFMPEG_LIB}.dll.a")
			message(FATAL_ERROR
				"Missing FFmpeg import library: "
				"${DTTR_FFMPEG_DIR}/lib/lib${DTTR_FFMPEG_LIB}.dll.a. "
				"Run toolchain/bootstrap first."
			)
		endif()

		if(NOT TARGET ${DTTR_FFMPEG_TARGET})
			add_library(${DTTR_FFMPEG_TARGET} SHARED IMPORTED)
			set_target_properties(${DTTR_FFMPEG_TARGET} PROPERTIES
				IMPORTED_IMPLIB "${DTTR_FFMPEG_DIR}/lib/lib${DTTR_FFMPEG_LIB}.dll.a"
			)
		endif()
	endforeach()

	set(DTTR_FFMPEG_DIR "${DTTR_FFMPEG_DIR}" PARENT_SCOPE)
	set(DTTR_FFMPEG_TARGETS ${DTTR_FFMPEG_TARGETS} PARENT_SCOPE)
	set(DTTR_FFMPEG_RUNTIME_DLLS ${DTTR_FFMPEG_RUNTIME_DLLS} PARENT_SCOPE)
endfunction()

function(dttr_use_windows_static_runtime target)
	cmake_parse_arguments(DTTR_TARGET "WIN32_RELEASE" "" "" ${ARGN})

	target_link_options("${target}" PRIVATE -static)
	if(DTTR_TARGET_WIN32_RELEASE)
		target_link_options("${target}" PRIVATE $<$<CONFIG:release>:-mwindows>)
	endif()
endfunction()

function(dttr_set_target_version target)
	target_compile_definitions("${target}" PRIVATE DTTR_VERSION="${DTTR_VERSION}")
endfunction()

function(dttr_add_icon_asset_command
	source
	ico
	png16
	png32
	png48
	png256
	window_png
	window_header
	header_symbol
	comment
)
	add_custom_command(
		OUTPUT
			"${ico}"
			"${png16}"
			"${png32}"
			"${png48}"
			"${png256}"
			"${window_png}"
			"${window_header}"
		COMMAND "${DTTR_RSVG_CONVERT}" -w 256 -h 256 -f png
			-o "${png256}"
			"${source}"
		COMMAND "${DTTR_RSVG_CONVERT}" -w 48 -h 48 -f png
			-o "${png48}"
			"${source}"
		COMMAND "${DTTR_RSVG_CONVERT}" -w 32 -h 32 -f png
			-o "${png32}"
			"${source}"
		COMMAND "${DTTR_RSVG_CONVERT}" -w 16 -h 16 -f png
			-o "${png16}"
			"${source}"
		COMMAND "${DTTR_RSVG_CONVERT}" -w 64 -h 64 -f png
			-o "${window_png}"
			"${source}"
		COMMAND "${DTTR_MAGICK}"
			"${png256}"
			"${png48}"
			"${png32}"
			"${png16}"
			-define icon:png-compression-size=1
			"${ico}"
		COMMAND "${DTTR_XXD}" -i -n "${header_symbol}"
			"${window_png}"
			"${window_header}"
		DEPENDS
			"${source}"
		COMMENT "${comment}"
		VERBATIM
	)
endfunction()

function(dttr_configure_icon_assets)
	find_program(DTTR_MAGICK magick REQUIRED)
	find_program(DTTR_RSVG_CONVERT rsvg-convert REQUIRED)
	find_program(DTTR_XXD xxd REQUIRED)

	set(DTTR_ICON_SOURCE "${CMAKE_SOURCE_DIR}/assets/icons/dttr.svg")
	set(DTTR_CONFIG_ICON_SOURCE "${CMAKE_SOURCE_DIR}/assets/icons/dttr_config.svg")
	set(DTTR_ICON_GENERATED_DIR "${CMAKE_BINARY_DIR}/generated/icons")
	set(DTTR_ICON_16_PNG "${DTTR_ICON_GENERATED_DIR}/dttr_16.png")
	set(DTTR_ICON_32_PNG "${DTTR_ICON_GENERATED_DIR}/dttr_32.png")
	set(DTTR_ICON_48_PNG "${DTTR_ICON_GENERATED_DIR}/dttr_48.png")
	set(DTTR_ICON_256_PNG "${DTTR_ICON_GENERATED_DIR}/dttr_256.png")
	set(DTTR_ICON_ICO "${DTTR_ICON_GENERATED_DIR}/dttr.ico")
	set(DTTR_ICON_RESOURCE "${DTTR_ICON_GENERATED_DIR}/dttr_icon.rc")
	set(DTTR_WINDOW_ICON_PNG "${DTTR_ICON_GENERATED_DIR}/dttr_window_icon.png")
	set(DTTR_WINDOW_ICON_HEADER "${DTTR_ICON_GENERATED_DIR}/dttr_window_icon.h")
	set(DTTR_CONFIG_ICON_16_PNG "${DTTR_ICON_GENERATED_DIR}/dttr_config_16.png")
	set(DTTR_CONFIG_ICON_32_PNG "${DTTR_ICON_GENERATED_DIR}/dttr_config_32.png")
	set(DTTR_CONFIG_ICON_48_PNG "${DTTR_ICON_GENERATED_DIR}/dttr_config_48.png")
	set(DTTR_CONFIG_ICON_256_PNG "${DTTR_ICON_GENERATED_DIR}/dttr_config_256.png")
	set(DTTR_CONFIG_ICON_ICO "${DTTR_ICON_GENERATED_DIR}/dttr_config.ico")
	set(DTTR_CONFIG_ICON_RESOURCE "${DTTR_ICON_GENERATED_DIR}/dttr_config_icon.rc")
	set(DTTR_CONFIG_WINDOW_ICON_PNG "${DTTR_ICON_GENERATED_DIR}/dttr_config_window_icon.png")
	set(DTTR_CONFIG_WINDOW_ICON_HEADER "${DTTR_ICON_GENERATED_DIR}/dttr_config_window_icon.h")
	file(MAKE_DIRECTORY "${DTTR_ICON_GENERATED_DIR}")
	file(CONFIGURE OUTPUT "${DTTR_ICON_RESOURCE}" CONTENT "1 ICON \"${DTTR_ICON_ICO}\"\n")
	file(CONFIGURE OUTPUT "${DTTR_CONFIG_ICON_RESOURCE}" CONTENT "1 ICON \"${DTTR_CONFIG_ICON_ICO}\"\n")

	dttr_add_icon_asset_command(
		"${DTTR_ICON_SOURCE}"
		"${DTTR_ICON_ICO}"
		"${DTTR_ICON_16_PNG}"
		"${DTTR_ICON_32_PNG}"
		"${DTTR_ICON_48_PNG}"
		"${DTTR_ICON_256_PNG}"
		"${DTTR_WINDOW_ICON_PNG}"
		"${DTTR_WINDOW_ICON_HEADER}"
		DTTR_WINDOW_ICON_PNG
		"Generating DttR app icon assets"
	)

	dttr_add_icon_asset_command(
		"${DTTR_CONFIG_ICON_SOURCE}"
		"${DTTR_CONFIG_ICON_ICO}"
		"${DTTR_CONFIG_ICON_16_PNG}"
		"${DTTR_CONFIG_ICON_32_PNG}"
		"${DTTR_CONFIG_ICON_48_PNG}"
		"${DTTR_CONFIG_ICON_256_PNG}"
		"${DTTR_CONFIG_WINDOW_ICON_PNG}"
		"${DTTR_CONFIG_WINDOW_ICON_HEADER}"
		DTTR_CONFIG_WINDOW_ICON_PNG
		"Generating DttR config icon assets"
	)

	add_custom_target(dttr_icon_assets
		DEPENDS
			"${DTTR_ICON_ICO}"
			"${DTTR_ICON_16_PNG}"
			"${DTTR_ICON_32_PNG}"
			"${DTTR_ICON_48_PNG}"
			"${DTTR_ICON_256_PNG}"
			"${DTTR_ICON_RESOURCE}"
			"${DTTR_WINDOW_ICON_PNG}"
			"${DTTR_WINDOW_ICON_HEADER}"
			"${DTTR_CONFIG_ICON_ICO}"
			"${DTTR_CONFIG_ICON_16_PNG}"
			"${DTTR_CONFIG_ICON_32_PNG}"
			"${DTTR_CONFIG_ICON_48_PNG}"
			"${DTTR_CONFIG_ICON_256_PNG}"
			"${DTTR_CONFIG_ICON_RESOURCE}"
			"${DTTR_CONFIG_WINDOW_ICON_PNG}"
			"${DTTR_CONFIG_WINDOW_ICON_HEADER}"
	)

	set(DTTR_ICON_ICO "${DTTR_ICON_ICO}" PARENT_SCOPE)
	set(DTTR_ICON_RESOURCE "${DTTR_ICON_RESOURCE}" PARENT_SCOPE)
	set(DTTR_CONFIG_ICON_ICO "${DTTR_CONFIG_ICON_ICO}" PARENT_SCOPE)
	set(DTTR_CONFIG_ICON_RESOURCE "${DTTR_CONFIG_ICON_RESOURCE}" PARENT_SCOPE)
	set(DTTR_ICON_GENERATED_DIR "${DTTR_ICON_GENERATED_DIR}" PARENT_SCOPE)
endfunction()

function(dttr_copy_runtime_files target output_dir)
	cmake_parse_arguments(DTTR_COPY "" "" "FILES" ${ARGN})

	add_custom_command(TARGET "${target}" POST_BUILD
		COMMAND ${CMAKE_COMMAND} -E make_directory "${output_dir}"
		COMMAND ${CMAKE_COMMAND} -E copy_if_different
			${DTTR_COPY_FILES}
			"${output_dir}"
		COMMAND_EXPAND_LISTS
		VERBATIM
	)
endfunction()

function(dttr_copy_modding_sdk_distribution target output_dir)
	add_dependencies("${target}" dttr_sdk_bundle_header dttr_pcdogs_generated_headers)

	get_property(DTTR_GENERATED_SDK_BUNDLE_HEADER GLOBAL PROPERTY DTTR_SDK_BUNDLE_HEADER)
	if(NOT DTTR_GENERATED_SDK_BUNDLE_HEADER)
		set(DTTR_GENERATED_SDK_BUNDLE_HEADER "${DTTR_SDK_BUNDLE_HEADER}")
	endif()

	if(NOT DTTR_SDK_SYMBOL_MANIFEST OR NOT DTTR_SDK_SYMBOL_SCHEMA)
		message(FATAL_ERROR "DttR SDK metadata paths are not configured")
	endif()

	set(DTTR_SELECTED_SDL3_INCLUDE_ROOT)
	foreach(DTTR_SDL3_INCLUDE_DIR IN LISTS SDL3_INCLUDE_DIRS)
		if(EXISTS "${DTTR_SDL3_INCLUDE_DIR}/SDL3/SDL.h")
			set(DTTR_SELECTED_SDL3_INCLUDE_ROOT "${DTTR_SDL3_INCLUDE_DIR}")
			break()
		endif()
	endforeach()

	if(NOT DTTR_SELECTED_SDL3_INCLUDE_ROOT)
		message(FATAL_ERROR
			"SDL3/SDL.h not found in SDL3_INCLUDE_DIRS='${SDL3_INCLUDE_DIRS}'. "
			"Required for modding SDK packaging."
		)
	endif()

	set(DTTR_CIMGUI_DIR "${CMAKE_SOURCE_DIR}/vendor/cimgui")
	set(DTTR_CIMGUI_IMGUI_DIR "${DTTR_CIMGUI_DIR}/imgui")
	set(DTTR_CIMGUI_BACKENDS_DIR "${DTTR_CIMGUI_IMGUI_DIR}/backends")
	set(DTTR_SDL3_LICENSE "${CMAKE_SOURCE_DIR}/.toolchain/sdl3/LICENSE.txt")

	set(DTTR_COPY_MODDING_SDK_LICENSE_COMMAND)
	if(EXISTS "${DTTR_SDL3_LICENSE}")
		set(DTTR_COPY_MODDING_SDK_LICENSE_COMMAND
			COMMAND ${CMAKE_COMMAND} -E copy_if_different
				"${DTTR_SDL3_LICENSE}"
				"${output_dir}/sdk/third_party/SDL3"
		)
	endif()

	add_custom_command(TARGET "${target}" POST_BUILD
		COMMAND ${CMAKE_COMMAND} -E rm -rf
			"${output_dir}/include"
			"${output_dir}/lib"
			"${output_dir}/cmake"
			"${output_dir}/scripts"
			"${output_dir}/blueprints"
			"${output_dir}/docs"
			"${output_dir}/sdk"
		COMMAND ${CMAKE_COMMAND} -E make_directory
			"${output_dir}/mods"
			"${output_dir}/sdk/include"
			"${output_dir}/sdk/lib"
			"${output_dir}/sdk/metadata"
			"${output_dir}/sdk/third_party/cimgui/imgui/backends"
			"${output_dir}/sdk/third_party/SDL3/include"
		COMMAND ${CMAKE_COMMAND} -E copy_if_different
			"${DTTR_GENERATED_SDK_BUNDLE_HEADER}"
			"${output_dir}/sdk/include"
		COMMAND ${CMAKE_COMMAND} -E copy_if_different
			"${DTTR_SDK_SYMBOL_MANIFEST}"
			"${DTTR_SDK_SYMBOL_SCHEMA}"
			"${output_dir}/sdk/metadata"
		COMMAND ${CMAKE_COMMAND} -E copy_if_different
			$<TARGET_LINKER_FILE:dttr_sidecar>
			"${output_dir}/sdk/lib"
		COMMAND ${CMAKE_COMMAND} -E copy_if_different
			"${CMAKE_SOURCE_DIR}/modules/sdk/DTTRSDKConfig.cmake"
			"${output_dir}/sdk"
		COMMAND ${CMAKE_COMMAND} -E copy_if_different
			"${DTTR_CIMGUI_DIR}/cimgui.h"
			"${DTTR_CIMGUI_DIR}/cimconfig.h"
			"${DTTR_CIMGUI_DIR}/cimgui_impl.h"
			"${DTTR_CIMGUI_DIR}/cimgui_impl_sdlgpu3.h"
			"${output_dir}/sdk/third_party/cimgui"
		COMMAND ${CMAKE_COMMAND} -E copy_if_different
			"${DTTR_CIMGUI_IMGUI_DIR}/imgui.h"
			"${DTTR_CIMGUI_IMGUI_DIR}/imconfig.h"
			"${DTTR_CIMGUI_IMGUI_DIR}/imgui_internal.h"
			"${DTTR_CIMGUI_IMGUI_DIR}/imstb_rectpack.h"
			"${DTTR_CIMGUI_IMGUI_DIR}/imstb_textedit.h"
			"${DTTR_CIMGUI_IMGUI_DIR}/imstb_truetype.h"
			"${output_dir}/sdk/third_party/cimgui/imgui"
		COMMAND ${CMAKE_COMMAND} -E copy_if_different
			"${DTTR_CIMGUI_BACKENDS_DIR}/imgui_impl_opengl3.h"
			"${DTTR_CIMGUI_BACKENDS_DIR}/imgui_impl_opengl3_loader.h"
			"${DTTR_CIMGUI_BACKENDS_DIR}/imgui_impl_sdl3.h"
			"${DTTR_CIMGUI_BACKENDS_DIR}/imgui_impl_sdlgpu3.h"
			"${DTTR_CIMGUI_BACKENDS_DIR}/imgui_impl_sdlgpu3_shaders.h"
			"${output_dir}/sdk/third_party/cimgui/imgui/backends"
		COMMAND ${CMAKE_COMMAND} -E copy_directory
			"${DTTR_SELECTED_SDL3_INCLUDE_ROOT}/SDL3"
			"${output_dir}/sdk/third_party/SDL3/include/SDL3"
		${DTTR_COPY_MODDING_SDK_LICENSE_COMMAND}
		COMMAND_EXPAND_LISTS
		VERBATIM
	)
endfunction()
