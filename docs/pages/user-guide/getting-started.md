...
- [Modding (__DTTR_DOCS_STABLE_VERSION__; ABI __DTTR_DOCS_STABLE_SDK_ABI__)](__DTTR_DOCS_MODDING_STABLE_DOWNLOAD_URL__) is the build that's required to load and run third-party modifications.
...
![DttR prompt asking where to load game files from](../assets/load-prompt.webp)
...
=== "Original CD"
    ...
    ![DttR loader showing an inserted game disc](../assets/load-disc.webp)

=== "Installed Copy"

    Choose `Open Directory` and select the installed game directory containing `pcdogs.exe`.

    ![Windows directory picker selecting an installed 102 Dalmatians directory](../assets/load-install.webp)

=== "ISO"

    Choose `Open ISO`, select the game ISO file, and click `Open`.

    ![Windows file picker selecting a 102 Puppies ISO image](../assets/load-iso.webp)
...
![102 Dalmatians title screen running through DttR](../assets/done.webp)
