...
typedef struct {
	uint32_t phase;			  // 0x0
	uint32_t failure_code;	  // 0x4
	uint32_t last_error;	  // 0x8
	uint32_t exception_code;  // 0xC
	uint32_t flags;			  // 0x10
	uint32_t helper_module;	  // 0x14
	uint32_t sidecar_module;  // 0x18
	uint32_t kernel32_module; // 0x1C
	uint32_t ntdll_module;	  // 0x20
	char dump_path[MAX_PATH]; // 0x24
} DTTR_BootstrapState;
...
typedef struct {
	...
	char dll_path[MAX_PATH]; // 0x0

	// Shellcode diagnostics.
	uint32_t status;	 // 0x104
	uint32_t module;	 // 0x108
	uint32_t last_error; // 0x10C

	// Original game entrypoint.
	uint32_t original_entry; // 0x110

	// Embedded kernel32.dll as UTF-16LE.
	WCHAR kernel32_name[sizeof(L"kernel32.dll") / sizeof(WCHAR)]; // 0x114

	// Embedded ASCII kernel32 exports.
	char loadlibraryex_name[sizeof("LoadLibraryExA")]; // 0x12E
	char exitthread_name[sizeof("ExitThread")];		   // 0x13D
	char getlasterror_name[sizeof("GetLastError")];	   // 0x148

	// Sidecar-resolved ExitThread pointer used after the entrypoint returns.
	uint32_t exitthread; // 0x158
