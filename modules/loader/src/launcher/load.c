...
static void s_resolve_sidecar_dll_path(char *out_path, size_t out_path_size) {
	...
	const bool copied = dttr_path_copy_sds(out_path, out_path_size, sidecar_path);
...
static LPVOID s_write_remote_payload(
	HANDLE process,
	const void *buffer,
	SIZE_T buffer_size
) {
	...
	LPVOID remote_buffer = DTTR_UNWRAP_WINAPI_EXISTS(VirtualAllocEx(
		process,
		NULL,
		buffer_size,
	...
	DTTR_UNWRAP_WINAPI_NONZERO(
		WriteProcessMemory(process, remote_buffer, buffer, buffer_size, NULL)
	);
...
void dttr_loader_inject_sidecar(const PROCESS_INFORMATION *child_info) {
	...
	LPVOID payload_buffer = s_write_remote_payload(
		child_info->hProcess,
		shellcode_buffer,
