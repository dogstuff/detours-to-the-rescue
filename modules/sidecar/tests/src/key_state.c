...
static void return_virtual_key_accepts_main_and_keypad_enter(void **state) {
	(void)state;
...
static void return_virtual_key_ignores_unpressed_enter_keys(void **state) {
	(void)state;
...
static void held_enter_that_opened_remap_is_debounced(void **state) {
	(void)state;

	assert_int_equal(dttr_inputs_controls_menu_pressed_button(VK_RETURN, 1, 1), -1);
}

static void released_then_pressed_enter_stays_bindable(void **state) {
	(void)state;

	assert_int_equal(dttr_inputs_controls_menu_pressed_button(VK_RETURN, 1, 0), VK_RETURN);
}

static void controls_menu_filter_leaves_escape_cancel_available(void **state) {
	(void)state;
